  ...
  */
  ...
  Canvas.prototype = {
    ...
    update: function (bitmap) {
      ...
      var scaledImageData = this.scaleImageData(imageData, this.scale)
      this.ctx.putImageData(scaledImageData, (bitmap.destLeft * this.scale) ^ 0,
        (bitmap.destTop * this.scale) ^ 0)
    },
    recalcScale () {
    ...
    scaleImageData: function (imageData, scale) {
      ...
      var scaleCanvas = $('<canvas>')
        .attr('width', (imageData.width * scale) ^ 0 || 1)
        .attr('height', (imageData.height * scale) ^ 0 || 1)[0]
