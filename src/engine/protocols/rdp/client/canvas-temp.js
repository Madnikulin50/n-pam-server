...
(function () {
  ...
  function decompress (params) {
    ...
    var inputPtr
    if (params.inputBufferLength === input.length) { inputPtr = params.inputBuffer } else {
    ...
    var outputPtr
    if (params.outputBufferLength === ouputSize) { outputPtr = params.outputBuffer } else {
  ...
  Canvas.prototype = {
    free: function () {
      if (this.cachedInputPtr) { Module._free(this.cachedInputPtr) }
      if (this.cachedOuputPtr) { Module._free(this.cachedOuputPtr) }
      this.cachedInputPtr = null
      this.cachedOuputPtr = null
